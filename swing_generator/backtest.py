...
Usage:
    ...
    python3 backtest.py --tf D              # one timeframe only (D, 1H, 4H, 3D or W)
...
# 3D gets 15 — the same ~6-9 week holding intent on a bar that holds three
# sessions, sitting between Daily's 30 (6 weeks) and Weekly's 13 (a quarter).
TIME_STOP_BARS = {'D': 30, '1H': 240, '4H': 60, '3D': 15, 'W': 13}
...
TF_SIGNAL_PARAMS = {
    ...
    '3D': {'refire_pct': REFIRE_PCT_3D, 'new_trend_pct': NEW_TREND_PCT_3D},
...
def backtest_instrument(ticker: str, name: str, group: str,
    ...
    # ── 3-DAY — mirror main.py: group the same finished daily bars ──
    # Same reasoning as Weekly below: built through the production helper so the
    # replay and the live run phase their bars identically, and the in-progress
    # group is dropped inside _resample_3d.
    if tf_filter in (None, '3D'):
        path = _cache_path(ticker)
        if os.path.exists(path):
            ...
            if len(d3_ma_periods) >= 2:
                ...
                three_day = add_signals(three_day, ma_periods=d3_ma_periods,
                                        **TF_SIGNAL_PARAMS['3D'])
                frames['3D'] = (three_day, d3_ma_periods)
...
def main():
    ...
    parser.add_argument('--tf',     choices=['D', '1H', '4H', '3D', 'W'], help='Only this timeframe')

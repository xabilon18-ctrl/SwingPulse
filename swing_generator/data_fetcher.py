...
# ---------------------------------------------------------------------------
# Second-source check: daily closes against the hourly feed
# ---------------------------------------------------------------------------
def cross_check_daily_hourly(instruments: list[dict], window: int = 60,
                             tolerance: float = 0.03, min_bad_days: int = 10) -> dict:
    """Flag instruments whose daily close keeps disagreeing with the last hourly
    close of the same date.

    Measured 2026-09-11 over 51,300 instrument-days: 96.3% agree within 1% and
    0.78% differ by more than 3%, so ONE bad day is noise. A flag needs
    `min_bad_days` of the last `window` shared dates past `tolerance` — which is
    what a feed that is wrong outright looks like. Skipped: crypto (the daily
    candle closes at 00:00 UTC, so its day never lines up with the hourly day),
    SPAIN35 (disagrees on 16.6% of days on a healthy feed), futures (`=F`: the
    daily bar is the SETTLEMENT and the last hourly bar is a later print — the
    first run flagged exactly six, all futures: COFFEE 46 of 60 days, OATMEAL,
    CORN, GASOLINE, COTTON, SUGAR) and anything whose hourly feed is a different
    contract (H4_SOURCE). Never raises for one instrument; a broken file is skipped.

    What it does NOT catch: an unadjusted split. Both caches are appended the
    same way, so both keep the pre-split prices and agree with each other (MNST
    and APH did not flag here). Splits are _history_mismatch's job, which
    compares the cache against a FRESH download.
    """
    from instruments import asset_class_of
    flagged, checked = {}, 0
    for inst in instruments:
        ticker = inst['ticker']
        if (asset_class_of(inst.get('group', '')) == 'Crypto' or inst.get('name') == 'SPAIN35'
                or ticker.endswith('=F') or h4_ticker(ticker) != ticker):
            continue
        dpath, hpath = _cache_path(ticker), _cache_path(ticker, suffix='1h')
        if not (os.path.exists(dpath) and os.path.exists(hpath)):
            continue
        try:
            daily = _read_cache(dpath)['Close']
            hourly = _read_cache(hpath)['Close']
        except Exception:
            continue
        hidx = pd.DatetimeIndex(hourly.index)
        if hidx.tz is not None:
            hidx = hidx.tz_convert('UTC').tz_localize(None)
        hourly_day = pd.Series(hourly.values, index=hidx.normalize()).groupby(level=0).last()
        didx = pd.DatetimeIndex(daily.index)
        if didx.tz is not None:
            didx = didx.tz_localize(None)
        daily_day = pd.Series(daily.values, index=didx.normalize())
        daily_day = daily_day[~daily_day.index.duplicated(keep='last')]
        common = daily_day.index.intersection(hourly_day.index)[-window:]
        if len(common) < 20:
            continue
        checked += 1
        diff = (hourly_day.loc[common] / daily_day.loc[common] - 1).abs().dropna()
        bad = diff[diff > tolerance]
        if len(bad) >= min_bad_days:
            flagged[inst['name']] = {
                'bad_days': int(len(bad)), 'days': int(len(diff)),
                'worst_pct': round(float(diff.max()) * 100, 1),
                'worst_date': str(diff.idxmax().date()),
            }
    return {
        'generated_at': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ'),
        'checked': checked, 'window_days': window,
        'tolerance_pct': round(tolerance * 100, 1), 'min_bad_days': min_bad_days,
        'flagged': flagged,
    }

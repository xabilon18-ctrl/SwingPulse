...
def write_output(df: pd.DataFrame, run_date: Optional[date] = None,
    ...
    output_dir     — override the default OUTPUT_DIR (for intraday pipeline)
    output_columns — override the default OUTPUT_COLUMNS column ordering
    """
    if run_date is None:
        run_date = date.today()
    ...
    try:
        ...
        out.to_excel(xlsx_path, index=False, sheet_name=str(run_date))
    print(f'  [Excel]  Saved → {xlsx_path}')

    # Attempt Google Sheets (daily pipeline only)

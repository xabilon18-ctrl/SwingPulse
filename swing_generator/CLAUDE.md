...
- **`data_checks.json`** (main.py step 2b, `data_fetcher.cross_check_daily_hourly`): flags an
...
- `style.css` — **v263** (v263: `.sc-state-pull`, `.sc-cost*`, `.tc-now*`, `.mh-data-warn`; `.mh-price-row`/`.mh-price-meta` wrap. v262: `.rot-*` wheel and `.lead-*` ranking styles; zone colours avoid --buy/--sell. v261: `[hidden]` rules for retired controls, `.sig-honesty-note`, amber `.sc-sig-warn`. v260: `.tf-switch-btn[hidden]` for per-tab timeframe buttons. v234: `.reel-nav` — up/down controls anchored to the right edge

...
- `style.css` — **v262** (v262: `.rot-*` wheel and `.lead-*` ranking styles; zone colours avoid --buy/--sell. v261: `[hidden]` rules for retired controls, `.sig-honesty-note`, amber `.sc-sig-warn`. v260: `.tf-switch-btn[hidden]` for per-tab timeframe buttons. v234: `.reel-nav` — up/down controls anchored to the right edge

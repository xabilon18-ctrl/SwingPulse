...
- `style.css` — **v266** (v266: `.reel-ladder*`, `.cf-step`, `.cf-pos`. v265: `.mh-data-warn` removed. v264: `.reel-trendline*`, `.reel-strip-lbl`. v263: `.sc-state-pull`, `.sc-cost*`, `.tc-now*`, `.mh-data-warn`; `.mh-price-row`/`.mh-price-meta` wrap. v262: `.rot-*` wheel and `.lead-*` ranking styles; zone colours avoid --buy/--sell. v261: `[hidden]` rules for retired controls, `.sig-honesty-note`, amber `.sc-sig-warn`. v260: `.tf-switch-btn[hidden]` for per-tab timeframe buttons. v234: `.reel-nav` — up/down controls anchored to the right edge

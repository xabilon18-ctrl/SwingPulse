...
@app.route('/api/signals-1h')
def api_signals_1h():
    """Return the latest 1H-only signal data (generated by main.py --timeframe 1h)."""
    ...
    df       = pd.read_csv(latest).fillna('')
    return jsonify({'date': date_str, 'data': df.to_dict(orient='records')})
